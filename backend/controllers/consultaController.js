...
const realizarConsulta = async (req, res) => {
  try {
    console.log('=== CONTROLLER REALIZARCONSULTA INICIADO ===');
    const { modulo_id, input } = req.body;
    console.log('Modulo ID recebido:', modulo_id);
    console.log('Input recebido:', input);
    console.log('Usuario ID:', req.user.id);

    if (!modulo_id || !input) {
      console.log('Erro: Modulo ou input faltando');
      return res.status(400).json({ error: 'Módulo e dados de entrada são obrigatórios' });
    }

    console.log('Chamando executarConsulta...');
    const resultado = await executarConsulta(req.user.id, modulo_id, input);
    console.log('Resultado da execucao:', resultado);
